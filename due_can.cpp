...
//arduino 1.5.2 doesn't init canbus so make sure to do it here. 
  ...
  //UPDATE: using four maiboxes for constant forwarding
  setNumTXBoxes(4);
...
uint8_t CANRaw::check_error(void)
{
  if((m_pCan->CAN_SR & 0b00000000110000101100001011000010)==0) return 1;
  else return 0;
...
/* Original version:
  ...
  The problem is that the setRXFilter() function calls findFreeRXMailbox(),
  which ALWAYS returns zero.
  Secondly, the function enable_interrupt() is already called within setRXFilter().
 */
  // A simple mailbox management for the starter
  setRXFilter(0, 0, 0, false);  // Use mailbox 0 for standard (11-bit) message IDs
  setRXFilter(1, 0, 0, true);   // Use mailbox 1 for extended (29-bit) message IDs
